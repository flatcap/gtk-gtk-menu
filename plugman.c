...
disable_plugin (const gchar *name)
{
	...
	if (plugin_menu) {
		...
		for (i = 0; i < g_menu_model_get_n_items (plugin_menu); i++) {
				if (g_menu_model_get_item_attribute (plugin_menu, i, "id", "s", &id) && g_strcmp0 (id, name) == 0) {
...
main (int argc, char **argv)
{
	...
	gtk_application_add_accelerator (GTK_APPLICATION (plug_man), "F11", "win.fullscreen", NULL);
